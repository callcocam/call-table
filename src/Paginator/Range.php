        ...
        for ($i = $pageRange['start']; $i <= $pageRange['end']; $i++) {
            ...
            if (($i == $pageRange['start']) && ($pageRange['prev'])) {
                if (null !== $this->bookends['start']) {
                    $this->links[] = sprintf("<li><a %s href='#' data-page='1'>%s</a></li>",$classOn,$this->bookends['start']);
                }
                if (null !== $this->bookends['previous']) {
                    $this->links[] = sprintf("<li %s><a href='#' data-page='%s'>%s</a></li>",$classOn, ($i - 1),$this->bookends['previous']);
                }
            }
            ...
            if (($i == $pageRange['end']) && ($pageRange['next'])) {
                if (null !== $this->bookends['next']) {
                   $this->links[] = sprintf("<li><a %s href='#' data-page='%s'>%s</a></li>",$classOn, ($i + 1),$this->bookends['next']);
                }
                if (null !== $this->bookends['end']) {
                   $this->links[] = sprintf("<li><a %s href='#' data-page='%s'>%s</a></li>",$classOn, $this->numberOfPages,$this->bookends['end']);
                }
            }
        }
    }
